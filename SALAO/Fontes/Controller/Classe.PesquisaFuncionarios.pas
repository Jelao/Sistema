...
type
  TPesquisaFuncionarios = class(TFuncionarios)
    ...
    public
      class procedure OpenTable(tblDepartamento,
...
class procedure TPesquisaFuncionarios.OpenTable(tblDepartamento,
                                                tblEndereco: TFDTable);
begin
  tblDepartamento.Connection     := DM_CADASTRO.CONN;
  tblEndereco.Connection         := DM_CADASTRO.CONN;

  tblDepartamento.Close;
  tblDepartamento.Open;
...
begin
  ...
  begin
    if Trim(Pesquisa.idFuncionarios) <> '' then
      sPesquisa := sPesquisa + ' AND idFuncionario = ' + QuotedStr(Pesquisa.idFuncionarios);
  end
  else
  begin
    if Trim(Pesquisa.idFuncionarios) <> '' then
      sPesquisa := ' Where idFuncionario = ' + QuotedStr(Pesquisa.idFuncionarios);
  end;
